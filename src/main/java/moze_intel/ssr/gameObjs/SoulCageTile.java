...
public class SoulCageTile extends TileEntity implements IInventory
{
    ...
    private boolean canEntitySpawn(EntityLiving ent)
    {
        ...
        if (TierHandler.getChecksRedstone(tier - 1) && redstoneActive == SSRConfig.INVERT_REDSTONE)
        {
            return false;
        }

        if (TierHandler.getChecksPlayer(tier - 1) && !isPlayerClose(this.xCoord, this.yCoord, this.zCoord))
        {
            return false;
        }
    ...
    {
        return stack != null && stack.getItem() == ObjHandler.SOUL_SHARD && Utils.isShardBound(stack) && Utils.getShardTier(stack) > 0;
    }
}
